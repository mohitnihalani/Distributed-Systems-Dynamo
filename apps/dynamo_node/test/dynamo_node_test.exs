defmodule DynamoNodeTest do
  ...
  test "Test Simple Put and Get in midst of Failure" do
    Emulation.init()
    Emulation.append_fuzzers([Fuzzers.delay(2)])
    ...
    client = spawn(:c1, fn ->
      ...
      {confirmation, _, coordinator_node} = DynamoNode.Client.put_request_node(client, "foo", 200, nil, :b)
      assert confirmation == :ok

      {:ok, ^client} = DynamoNode.Client.fail_node(client, coordinator_node)
  ...
  test "Test Get with Drop Probability and Failures" do
    Emulation.init()
    ...
    client = spawn(:c1, fn ->
      ...
      {:ok, {_, [10 | _tail ], [vhead | _vtail]}} = DynamoNode.Client.get_request(client, "foo", :a)
      assert DynamoNode.Client.put_request(client, "foo", 90, vhead, :e) == {:ok, client}
      {:ok, {_, [value | _ ], [vhead | vtail]}} = DynamoNode.Client.get_request(client, "foo", :b)
      assert value = 90
      ######
      {:ok, {_, [100 | _tail ], [vhead | _vtail]}} = DynamoNode.Client.get_request(client, "lsk", :f)
      assert DynamoNode.Client.put_request(client, "lsk", 18, vhead, :f) == {:ok, client}
